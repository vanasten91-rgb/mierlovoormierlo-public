...
final class MvM_Hub4_Hub_Security {
	private const CAPS_SCHEMA_OPTION = 'mvm_hub4_compat_caps_schema';
	private const SECURE_VERSION_OPTION = 'mvm_hub4_compat_security_version';
	...
	public static function activate(): void {
		if ( ! class_exists( 'MvM_Hubs_V3_Security' ) ) {
	...
	public static function maybe_sync_capabilities(): void {
		if ( class_exists( 'MvM_Hubs_V3_Security' ) || self::is_draft_preview() ) {
			return;
		}
		if ( MVM_HUB4_HUB_CAPS_SCHEMA !== (string) get_option( self::CAPS_SCHEMA_OPTION, '' ) ) {
			self::sync_capabilities();
			return;
		}
		if ( MVM_HUB4_VERSION !== (string) get_option( self::SECURE_VERSION_OPTION, '' ) ) {
			update_option( self::SECURE_VERSION_OPTION, MVM_HUB4_VERSION, false );
		}
	}
	...
	public static function sync_capabilities(): bool {
		...
		update_option( self::CAPS_SCHEMA_OPTION, MVM_HUB4_HUB_CAPS_SCHEMA, false );
		update_option( self::SECURE_VERSION_OPTION, MVM_HUB4_VERSION, false );
		return true;
	}
	...
	public static function register_rest_routes(): void {
		self::register_context_route( 'mvm-hub4/v1' );
		if ( ! class_exists( 'MvM_Hubs_V3_Security' ) ) {
			self::register_context_route( 'mvm-hubs/v3' );
		}
	}

	private static function register_context_route( string $namespace ): void {
		register_rest_route(
			$namespace,
			'/context',
			array(
				...
				'callback'            => static function (): WP_REST_Response {
					...
					foreach ( array( 'business', 'editorial', 'admin' ) as $hub ) {
						if ( MvM_Hub4_Hub_Security::can_access( $hub ) ) {
							$allowed[] = $hub;
						}
					}
					$sections = array();
					foreach ( $allowed as $hub ) {
						$sections[ $hub ] = MvM_Hub4_Hub_Security::allowed_sections( $hub );
					}
					$response = new WP_REST_Response(
						array(
							'version'     => MVM_HUB4_VERSION,
							'primary_hub' => MvM_Hub4_Hub_Security::primary_hub(),
							'hubs'        => $allowed,
							'sections'    => $sections,
							'permissions' => array(
								'submit_editorial' => MvM_Hub4_Hub_Security::can_submit_editorial(),
								'review'           => MvM_Hub4_Hub_Security::can_review(),
								'vacancy_review'   => MvM_Hub4_Hub_Security::can_review_vacancies(),
								'moderate'         => MvM_Hub4_Hub_Security::can_moderate(),
								'team_planning'    => MvM_Hub4_Hub_Security::can_view_team_planning(),
							),
						),
						200
					);
